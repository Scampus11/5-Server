...
namespace SMS
{
    public partial class Access_List : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                if (Request.QueryString["Id"] != null)
                {
                    ...
                    //cmd.CommandText = "select distinct S.Id, S.StudentId, (S.FirstName + ' '+S.FatherName) Student_Name, stuff((select ',' + Access_Group_ID from Access_List where StudentId=sm.StudentId for xml path('')),1,1,'') as Access_Group_Id ,stuff((select ',' + Cast(ID as Nvarchar(50)) from Access_List where StudentId=sm.StudentId for xml path('')),1,1,'') as A_Id from Access_List SM Right join tblstudent S on SM.StudentId=S.StudentId where S.StudentId= '" + Request.QueryString["Id"].ToString() + "'";
                    cmd.CommandText = "select distinct S.Id, S.StudentId, (S.FirstName + ' '+S.FatherName) Student_Name, stuff((select ',' + Access_Group_ID from Access_List where StudentId=sm.StudentId for xml path('')),1,1,'') as Access_Group_Id , stuff((select ',' + Cast(ID as Nvarchar(50)) from Access_List where StudentId=sm.StudentId for xml path('')),1,1,'') as A_Id, stuff((select ',' + Session_ID from Access_List where StudentId=sm.StudentId for xml path('')),1,1,'') as Session_ID from Access_List SM Right join tblstudent S on SM.StudentId=S.StudentId where S.StudentId= '" + Request.QueryString["Id"].ToString() + "'";
        ...
        protected void FillGrid(string sortExpression = null)
        {
            ...
            try
            {
                ...
                ////cmd.CommandText = "select Distinct S.ID,s.StudentId,(s.FirstName +'' +s.FatherName) Student_Name ,stuff ( ( select ','+ Access_Group_Name from Access_List a inner join tbl_Access_Group b on b.Id = a.Access_Group_ID where StudentId=S.StudentID for xml path('') ),1,1,'' ) Access_Group_ID from tblstudent s left join Access_List ac on ac.StudentId=s.StudentID";
                //cmd.CommandText = "select Distinct S.ID,s.StudentId,(s.FirstName +'' +s.FatherName) Student_Name , stuff ( ( select ','+ Access_Group_Name from Access_List a inner join tbl_Access_Group b on b.Id = a.Access_Group_ID where StudentId=S.StudentID for xml path('') ),1,1,'' ) Access_Group_ID ,stuff ( ( select ','+ Session_Name from Access_List a inner join tbl_Student_Session b on b.Session_Id = a.Session_Id where StudentId=S.StudentID for xml path('') ),1,1,'' ) Session_Name from tblstudent s left join Access_List ac on ac.StudentId=s.StudentID";
        ...
        {
            Response.Redirect("Access_List.aspx?Insert=Insert Department");
        }
        protected void linkEdit_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("Access_List.aspx?Id=" + Ids.Text + "");
        }
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            if (Validation())
            {
                ...
                Response.Redirect("Access_List.aspx");
            ...
            if (Validation())
            {
                ...
                foreach (RepeaterItem item in RepeatReader.Items)
                {
                    ...
                    if (chkReader.Checked)
                    {
                        ...
                        cmd1.CommandText = "select Count(*) A_Count from Access_List where StudentId='" + txtApplication_No.Text + "' and Access_Group_Id='" + lblId.Text + "'";
                        cmd1.Connection = con;
                        ...
                        {
                            cmd.CommandText = "UPDATE [Access_List] SET  [Access_Group_ID] = '" + lblId.Text + "' WHERE StudentId='" + Request.QueryString["Id"].ToString() + "' and [Access_Group_ID] = '" + lblId.Text + "'";
                            cmd.Connection = con;
                            cmd.ExecuteNonQuery();
                        }
                        //ScriptManager.RegisterStartupScript(this, GetType(), "", "alert('Insert Successfully!!');", true);
                    ...
                    else
                    {
                        ...
                        if (ds.Tables[0].Rows[0]["A_Count"].ToString() != "0")
                        {
                            cmd.CommandText = "Delete from [Access_List] WHERE StudentId='" + Request.QueryString["Id"].ToString() + "' and [Access_Group_ID] = '" + lblId.Text + "'";
                            cmd.Connection = con;
                            cmd.ExecuteNonQuery();
                        }
                ...
                foreach (RepeaterItem item in rptSession.Items)
                {
                    ...
                    if (chkSession.Checked)
                    {
                        ...
                        cmd1.CommandText = "select Count(*) A_Count from Access_List where StudentId='" + txtApplication_No.Text + "' and Session_Id='" + lblId.Text + "'";
                        cmd1.Connection = con;
                        SqlDataAdapter da = new SqlDataAdapter(cmd1);
                        DataSet ds = new DataSet();
                        ...
                        if (ds.Tables[0].Rows[0]["A_Count"].ToString() == "0")
                        {
                            cmd3.CommandText = "INSERT INTO [Access_List] ([StudentId] ,[Student_Name],[Session_ID]) VALUES ('" + txtApplication_No.Text + "','" + txtDescription.Text + "','" + lblId.Text + "')";
                            cmd3.Connection = con;
                            cmd3.ExecuteNonQuery();
                        }
                        else
                        {
                            cmd3.CommandText = "UPDATE [Access_List] SET  [Session_ID] = '" + lblId.Text + "' WHERE StudentId='" + Request.QueryString["Id"].ToString() + "' and [Session_ID] = '" + lblId.Text + "'";
                            cmd3.Connection = con;
                            cmd3.ExecuteNonQuery();
                        }
                        //ScriptManager.RegisterStartupScript(this, GetType(), "", "alert('Insert Successfully!!');", true);
                    ...
                    else
                    {
                        ...
                        if (ds.Tables[0].Rows[0]["A_Count"].ToString() != "0")
                        {
                            cmd3.CommandText = "Delete from [Access_List] WHERE StudentId='" + Request.QueryString["Id"].ToString() + "' and [Session_ID] = '" + lblId.Text + "'";
                            cmd3.Connection = con;
                            cmd3.ExecuteNonQuery();
                        }
        ...
        protected void linkDelete_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = "Delete from Access_List where StudentId= '" + Ids.Text + "'";
